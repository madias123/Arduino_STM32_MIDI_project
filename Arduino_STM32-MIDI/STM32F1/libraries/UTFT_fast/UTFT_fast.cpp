...
//		cbi(P_RS, B_RS); SPI.transfer(VL);	return;
		LCD_Writ_Bus(0x00,VL,display_transfer_mode);
}
...
void UTFT::LCD_Write_DATA(char VH,char VL)
{
	...
	{
//	sbi(P_RS, B_RS);SPI.transfer(VH);SPI.transfer(VL);	return;
...
//		sbi(P_RS, B_RS); SPI.transfer(VL);	return;
		LCD_Writ_Bus(0x01,VL,display_transfer_mode);
}
...
void UTFT::fillRect(int x1, int y1, int x2, int y2)
{
	...
	else
	{
		...
		else
		{
			...
			cbi(P_CS, B_CS);
		    setXY(x1, y1, x2, y2);
			for (uint32 i=0; i<((long(x2-x1)+1)*(long(y2-y1)+1)); i++)
			{
