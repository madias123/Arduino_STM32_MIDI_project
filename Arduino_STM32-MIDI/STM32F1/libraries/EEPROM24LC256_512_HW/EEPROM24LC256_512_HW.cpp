...
#include "EEPROM24LC256_512_HW.h"

HardWire HWIRE_eeprom(1,I2C_FAST_MODE); // I2c2
...
uint8_t EEPROM256_512::readByte(unsigned int addr)
{
    ...
    HWIRE_eeprom.beginTransmission(fulladdr);//begin transmission on the wire
    //32768 bytes... addressed by 2 bytes (16bits)
    HWIRE_eeprom.write((int)(addr >> 8));   // MSB write upper 8 bits to wire (by bit shifting to right)
    HWIRE_eeprom.write((int)(addr & 0xFF)); // LSB write lower 8 bits to wire (by bit & by 0xFF)
    HWIRE_eeprom.endTransmission();//end the transmission on the wire
    HWIRE_eeprom.requestFrom((uint8_t)fulladdr,(uint8_t)1);//request 1 byte from EEPROM (memory location sent above)
    if (HWIRE_eeprom.available()) data = HWIRE_eeprom.read();
    return data;
}//end readByte

void EEPROM256_512::writeByte(unsigned int addr, uint8_t data)
{
	HWIRE_eeprom.beginTransmission(fulladdr);//begin transmission on wire
    //32768 bytes... addressed by 2 bytes (16bits)
    HWIRE_eeprom.write((int)(addr >> 8));   // MSB write upper 8 bits to wire (by bit shifting to right)
    HWIRE_eeprom.write((int)(addr & 0xFF)); // LSB write lower 8 bits to wire (by bit & by 0xFF)
    HWIRE_eeprom.write(data);//write data byte
    HWIRE_eeprom.endTransmission();//end transmission on wire
...
void EEPROM256_512::readMiniBuffer(unsigned int addr)
{
    ...
    HWIRE_eeprom.requestFrom((uint8_t)fulladdr,(uint8_t)(MINIBUFFER));//request 1 byte from EEPROM (memory location sent above)
    while (HWIRE_eeprom.available())
    {
        minibuffer[ct] = HWIRE_eeprom.read();
        ct++;
    }
}//end readMiniBuffer
...
void EEPROM256_512::writeMiniBuffer(unsigned int addr)
{
    ...
    while(ct<MINIBUFFER)
    {
		HWIRE_eeprom.write((uint8_t)minibuffer[ct]);
		ct++;
    }  
    HWIRE_eeprom.endTransmission();
	delay(5);
}//end writeMiniBuffer
